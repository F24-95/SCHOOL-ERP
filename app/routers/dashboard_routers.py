...
from app.core.enums import AssignmentStatus, ExamStatus, UserRole
from app.dependencies import (
...
async def get_student_dashboard(
    ...
    current_session = (
        db.query(AcademicSession).filter(AcademicSession.is_current).first()
    )
    ...
    upcoming_assignments = (
        ...
        .filter(
            ...
            Assignment.due_date >= datetime.now(UTC).date(),
    ...
    upcoming_exams = (
        ...
        .filter(
            ...
            Exam.exam_date >= datetime.now(UTC).date(),
    ...
    return {
        "student": {
            ...
            "class": student_class.classroom.display_name
...
async def get_teacher_dashboard(
    ...
    teacher_subjects = (
        ...
        .filter(
            ...
            TeacherSubject.academic_sessions_id == current_session.session_code,
        )
        .all()
    )
    ...
    return {
        ...
        "recent_activity": {
            "last_7_days_classes": db.query(DailyClass)
            .filter(
                DailyClass.teacher_subject_id.in_(teacher_subject_ids),
...
async def get_admin_dashboard(
    ...
    total_user = db.query(User).filter(User.is_active).count()
    total_students = db.query(StudentProfile).filter(StudentProfile.is_active).count()
    total_teachers = db.query(TeacherProfile).filter(TeacherProfile.is_active).count()
    total_classes = db.query(ClassRoom).filter(ClassRoom.is_active).count()
    total_subjects = db.query(Subject).filter(Subject.is_active).count()

    # Current session statistics
    if current_session:
        session_classes = (
            db.query(ClassRoom)
            .filter(ClassRoom.academic_sessions_id == current_session.session_code)
            .count()
        )
        session_students = (
            db.query(StudentClass)
    ...
    # Recent activity
    recent_user = db.query(User).order_by(User.created_at.desc()).limit(5).all()
