...
from app.model import ClassRoom, ClassSubject, Subject, TeacherSubject, User
from app.schemas import (
    ClassSubjectCreate,
...
# ============================================================
# CLASS SUBJECT MAPPING (must be before parameterized routes)
# ============================================================
...
async def assign_subject_to_class(
    ...
    classroom = (
        db.query(ClassRoom)
        .filter(ClassRoom.class_code == mapping_data.classroom_id)
        .first()
    )
    if not classroom:
        raise HTTPException(
    ...
    subject = (
        db.query(Subject)
        .filter(Subject.subject_code == mapping_data.subject_id)
        .first()
    )
    if not subject:
        raise HTTPException(
...
async def get_subjects_for_class(
    ...
    query = (
        db.query(Subject)
        .join(ClassSubject, ClassSubject.subject_id == Subject.subject_code)
        .filter(ClassSubject.classroom_id == classroom_id)
    )
...
async def remove_class_subject(
    ...
    teacher_subject = (
        ...
        .filter(TeacherSubject.class_subject_id == mapping_id)
        .first()
    )
    if teacher_subject:
        raise HTTPException(
...
# ============================================================
# SUBJECT CRUD (parameterized routes)
# ============================================================
...
async def get_subject(
    ...
    """Get subject by ID."""
    subject = db.query(Subject).filter(Subject.subject_code == subject_id).first()
    if not subject:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Subject not found",
